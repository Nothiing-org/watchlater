...
const AddVideoForm: React.FC<AddVideoFormProps> = ({ onAdd, isLoading }) => {
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1 relative">
          <input
            ...
            disabled={isLoading}
          />
          {error && <span className="absolute -bottom-6 left-2 text-[10px] font-bold text-black label-wide">{error}</span>}
        </div>
        <button
          type="submit"
